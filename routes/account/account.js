...
const debug = require('debug')('rsvega-restapi:account')
...
router.get('/rsvega/account/user-id/:user_id/is-mule/active/random', (req, res) => {
    pool.get_connection(qb => {
        qb.query('SELECT * FROM `account` WHERE `is_mule` = 1 AND `user_id` = ' + qb.escape(req.params.user_id) + ' AND `last_update` >= NOW() - INTERVAL 1 MINUTE ORDER BY RAND() LIMIT 1', (err, rows) => {
            qb.release()
            if (err) {
                debug(err)
                return res.json([err])
